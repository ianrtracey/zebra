...
const createStoreWithThunkMiddleware = applyMiddleware(thunkMiddleware)(
  createStore
);
export const makeStore = (reduxState, enhancer) =>
  createStoreWithThunkMiddleware(
    combineReducers(thisReduxApi.reducers),
    reduxState
  );
...
const mapStateToProps = (endpointNames, reduxState) => {
  ...
  for (let i in endpointNames) {
    ...
    props[`${endpointNames[i]}Actions`] =
      thisReduxApi.actions[endpointNames[i]];
  }
  return props;
};

export const withReduxEndpoints = (PageComponent, endpointNames) =>
  connect(mapStateToProps.bind(undefined, endpointNames))(PageComponent);
// Define custom endpoints/providers here:
export const withEvents = PageComponent =>
  withReduxEndpoints(PageComponent, ["events"]);
